...
class OrderItem:
    def __init__(self, id=None, order_id=None, product_id=None, quantity=1, item_price=0.0, date_str=None):
        self.id = id
    ...
    def save(self):
        ...
        if self.id is None:
            cursor.execute(
                ...
                (self.date_str, self.order_id, self.product_id, self.quantity, self.item_price)
            )
            self.id = cursor.lastrowid
        else:
            cursor.execute(
                "UPDATE issue_details SET date=?, codeissuance=?, codebook=?, quantity=?, price=? WHERE codeissuedetails=?",
                (self.date_str, self.order_id, self.product_id, self.quantity, self.item_price, self.id)
            )
        conn.commit()
        conn.close()
